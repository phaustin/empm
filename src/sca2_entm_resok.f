...
c but resolving the Kolmogorov scale, using the real Dm Dtemp
        ...
        Parameter(BL=5.,BL_inte=5.,teta=.0072,Dtur=.184,
     $  Dm=2.747e-5,
     1  Dtemp=2.1e-5)
...
c
  481   continue
	...
	if(MOD(iteration,iteration1).eq.0)then
 	 ...
 	 write(62)time_time,(time_r(i)*1.e6,i=1,1700)
 	 write(67)time_time,(time_s(i),i=1,1700)
         write(68)time_time,(time_x(i),i=1,1700)
	 write(69)ndrop,(index_drop(i),i=1,ndrop)
	endif
...
c
     ...
     $  igrid)
c
c add the pdf calculations for supersaturation raion base on droplets
c
	ncut=0
	nacdrop=ndrop
	do i=1,ndrop
	  k=i-ncut
	  if(dsize(i).lt.1e-6)then
	     ncut=ncut+1
	     goto 978
	  endif
	  pjw=x(i)/dx
...
C*       call average(th5_pdfave,th5_pdf,nbin,ip,mreal,mip)
         ...
         call prob(1,nacdrop,th6_min,th6_max,th6,th6_pdf
