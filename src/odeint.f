     ...
     $  nbad,derivs,rkqs)
      ...
      data id/'$RCSfile: odeint.f,v $'/
      data revision/'$Revision: 1.5 $'/
      data revdate/'$Date: 1994/03/14 00:34:14 $'/
      INTEGER nbad,nok,nvar,KMAXX,MAXSTP,NMAX
      REAL*8 eps,h1,hmin,x1,x2,ystart(nvar),TINY
      EXTERNAL derivs,rkqs
      PARAMETER (MAXSTP=10000,KMAXX=200,TINY=1.e-30,nmax=100)
...
c$$$  this common block allows intermediate steps to be stored
c$$$
      if(nmax .ne. idimen) then
          print *,"error in odeint:  nmax= ",nmax," idimen= ",idimen
          stop
      endif
      x=x1
      h=sign(h1,x2-x1)
