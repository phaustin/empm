...
c but not resolving the Kolmogorov scale
        ...
        Parameter(BL=5.,BL_inte=5.,teta=.01,Dtur=.184,
     $  Dm=4.5e-5,
     1  Dtemp=4.5e-5)
...
c but resolving the Kolmogorov scale, using the real Dm Dtemp
C	Parameter(BL=20.,BL_inte=20.,teta=.012,Dtur=0.054,
C    $  Dm=2.747e-5,
C    1  Dtemp=2.1e-5)
...
c
	...
	open(67,file='stime.dat',form='unformatted')
...
c
  481   continue
	...
	if(MOD(iteration,iteration1).eq.0)then
	  time_time=iteration*t_use
	  write(62)time_time,(time_r(i)*1.e6,i=1,1000)
	  write(67)time_time,(time_s(i),i=1,1000)
	endif
