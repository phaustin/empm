...
c but resolving the Kolmogorov scale, using the real Dm Dtemp
 	Parameter(BL=20.,BL_inte=20.,teta=.012,Dtur=0.054,
     $  Dm=2.747e-5,
     1  Dtemp=2.1e-5)
...
c	parameter(temp_cloud=293.4, temp_parcel=293.4)
	...
	DIMENSION SA1VA1(mip), SA1VA1ave(mip)
	DIMENSION SA1VA2(mip), SA1VA2ave(mip)
        DIMENSION SA1VA3(mip), SA1VA3ave(mip)
        DIMENSION SA1VA4(mip), SA1VA4ave(mip)
        DIMENSION SA1VA5(mip), SA1VA5ave(mip)

        DIMENSION SA2VA1(mip), SA2VA1ave(mip)
        DIMENSION SA2VA2(mip), SA2VA2ave(mip)
        DIMENSION SA2VA3(mip), SA2VA3ave(mip)
        DIMENSION SA2VA4(mip), SA2VA4ave(mip)
        DIMENSION SA2VA5(mip), SA2VA5ave(mip)
...
c modified 05-08-95
c
	real*8 time_r(10000),time_qv(10000), time_T(10000),
     $  time_x(10000),time_s(10000)
c
c modified 05-09-95
c
	integer index_drop(5000)
Creal*8 bl_new
...
c
      ...
      open(unit=56,file="splinepoints.input",access='sequential',
     $     status='old',form='formatted')

      read(56,*) (((splinearray(i,ii,iii),i=1,ncats),ii=1,nvecs),
     $     iii=1,npoints)
...
c.
        ndrop_old=ndrop
	write(*,*)'before assign'
        call assign(ndrop,ngrid,jcell_pop,dx
...
c.
c. set previouly read unused droplet property to zero
c.
        do i=ndrop+1,ndrop_old
        rnr(i)=0.
        csize(i)=0.
        dsize(i)=0.
        icat(i)=0.
...
c
 ...
 5500   continue
	...
	call fdtime(t_prime, psigma,t_mean)
...
c
	...
	call intpol(time0,vel_ver,MI,parry,heiarry,p_e,Z)
 	...
 	open(76,file='entm.dat')
        write(76,4646)mreal,t_prime,t_entm,p_e,qv_e,t_mean
 4646   format(I12,2x,5(2x,e15.6))

c
c set velocity = 0. after desired entrainement event
c (note: must put this after the intrepolation)
c
Cif(t_entm.gt.125)then
Cvel_ver=0.
Cendif
...
c.
	call domain(psigma,ngrid,Xn,spoint,fpoint)
...
c
	call assign2(ndrop_used,nccn,m2,m3,jcell_pop,
     $  dx,m1,x,bl,igrid)
...
C6007   A(I,2)=A(I+NGRID,2)
 ...
 6020      CONTINUE
	if(iteration.ge.0)goto 1234

	open(121,file='ic.input')
	read(121,*)ndrop,press0,height0
	write(*,*)'ndrop',ndrop,'press0',press0,height0
	press=press0
	height=height0
	open(122,file='cdrop.input')
	do i=1,ndrop
	read(122,1222)nij(i),dsize(i),csize(i),rnr(i),x(i),icat(i)
 1222 	format(1I5,1x,4(e12.6,1x),1I5)
Cwrite(*,1222)(i),dsize(i),csize(i),rnr(i),x(i),icat(i)
	enddo

	open(123,file='cpro.input')
	do i=m1,m4
	read(123,1233)a(I,1),a(I,2)
 1233	format(2(1x,f12.6))
...
c
        do 30 j=1,ngrid
         m=j
c  write(*,*)m,index(m)
          dx_map(index(m))=DBLE(m-index(m))*dx
...
c
	ncat=31
	if(mod(iteration,10).gt.0)goto 481
Cif(mod(iteration,1000).gt.0)goto 481
Cif(mod(iteration,1).gt.0)goto 481
...
cwrite(85,*)'real=',mreal,'ite=',iteration
cwrite(*,*)'before dgm'
...
c if((MOD(i,40).eq.0).and.(MOD(iteration,500)
c   $     .eq.0))then
CCif(MOD(iteration,1000).eq.0)then
CC   i_index=index_drop(i)
c itime=int(float(i_index)/40.)
CC   itime=i_index
CC   time_r(itime)=radius
CC   time_qv(itime)=A(j_cell,1)
CC    time_T(itime)=A(j_cell,2
CC   time_x(itime)=x(i)
CC   time_s(itime)=Sus(j_cell)
CCendif
...
Cif(MOD(iteration,1000).eq.0)then
Ctime_time=iteration*t_use
Cwrite(62,483)time_time,(time_r(i),i=1,3200,40)
C       write(67,483)time_time,(time_r(i),i=3200,6396,40)
C       write(68,483)time_time,(time_r(i),i=6400,7527,40)
Cwrite(63,483)time_time,(time_qv(i),i=1,3200,40)
Cwrite(64,483)time_time,(time_T(i),i=1,3200,40)
Cwrite(65,483)time_time,(time_x(i),i=1,3200,40)
Cwrite(66,483)time_time,(time_s(i),i=1,3200,40)
Cwrite(69,483)time_time,(time_s(i),i=3200,6396,40)
Cwrite(71,483)time_time,(time_s(i),i=6400,7527,40)
Cendif
C483   format(1x,f10.4,80(1x,e10.4))
...
c
     ...
     $  (T.ge.t_entm))then
cwrite(88,*)'T',t,'t_entm',t_entm,iteration
	goto 5500
	endif
...
c
	...
	CALL PROB(M1,M4,pNmin1,pNmax1,SA1,PDF1,SCALAR1,NBIN,Ip,mip,
     $  igrid)
	CALL PROB(M1,M4,pNmin2,pNmax2,SA2,PDF2,SCALAR2,NBIN,Ip,mip,
     $  igrid)
...
C
        ...
        CALL PHI(M1,M4,SA1,s1w1,s1w2,s1w4,s1w6,igrid)
C
...
C	write(85,*)'THIS IS'
        SA1VA1(ip)=s1w1
        SA1VA2(ip)=s1w4/s1w2**2.
        SA1VA3(ip)=s1w6/s1w2**3.

        CALL PHI(M1,M4,SA2,s2w1,s2w2,s2w4,s2w6,igrid)
        SA2VA1(ip)=s2w1
        SA2VA2(ip)=s2w4/s2w2**2.
        SA2VA3(ip)=s2w6/s2w2**3.
c
c DISSIPATION a subroutine to calculate the dissipation rate
c and the correlation.
c
        CALL DISSIPATION(M1,M4,SA1,s1d4,s1d5,DX,Dm,igrid)
        SA1VA4(ip)=s1d4
        SA1VA5(ip)=s1d5

        CALL DISSIPATION(M1,M4,SA2,s2d4,s2d5,DX,Dtemp,igrid)
        SA2VA4(ip)=s2d4
        SA2VA5(ip)=s2d5
c
...
c
        CALL AVERAGE_1(SA1VA1ave,SA1VA1,SA1VA2ave
     1  ,SA1VA2,SA1VA3ave,SA1VA3,SA1VA4ave,SA1VA4
     2  ,SA1VA5ave,SA1VA5,Ip,Mreal,mip)

        CALL AVERAGE_1(SA2VA1ave,SA2VA1,SA2VA2ave
     1  ,SA2VA2,SA2VA3ave,SA2VA3,SA2VA4ave,SA2VA4
     2  ,SA2VA5ave,SA2VA5,Ip,Mreal,mip)
...
C*       call prob(m1,m4,th6_min,th6_max,th6,th6_pdf
C*    1  ,th6_scalar,nbin,ip,mip)
C*       call average(th6_pdfave,th6_pdf,nbin,ip,mreal,mip)
...
c for scalar1
c
 	OPEN (unit=10, file='s1stat.dat')
        WRITE(10,560)T,SA1VA1ave(ip),SA1VA2ave(ip),
     1  SA1VA3ave(ip),SA1VA4ave(ip),SA1VA5ave(ip)
 560    FORMAT(1x,E8.3,5(2x,E12.4))
c
c for scalar2
c
        OPEN (unit=9, file='s2stat.dat')
        WRITE(9,561)T,SA2VA1ave(ip),SA2VA2ave(ip),
     1  SA2VA3ave(ip),SA2VA4ave(ip),SA2VA5ave(ip)
 561    FORMAT(1x,E8.3,5(2x,E12.4))
...
C       write(38,3000)'#,realization=',mreal
	...
	T=ip*t_use*iteration1
     ...
     $   ,rmean(ip),rstd(ip),actndrop(ip)
	enddo
 388	format(19(1x,e14.6),1x,1i10)
...
C*       open(44,file='sus.pdf',form='unformatted')
...
C*       WRITE(44)(th6_pdfave(jj,K),k=1,nbin+1)
C*       END DO
...
C***************************************************************************
        ...
        SUBROUTINE PHI(M1,M4,BB,PHIRMS,PHIPRIM2,PHIPRIM4,PHIPRIM6,igrid)
	integer*4 igrid
        real*8 BB(0:igrid)
        G1=0.
        DO I=M1,M4
        ...
        PHIPRIM6=0.
        DO I=M1,M4
        PHIPRIM=PHIPRIM+(BB(I)-PHIMEAN)
        PHIPRIM2=PHIPRIM2+((BB(I)-PHIMEAN))**2.
        PHIPRIM4=PHIPRIM4+((BB(I)-PHIMEAN))**4.
        PHIPRIM6=PHIPRIM6+((BB(i)-PHIMEAN))**6.
        END DO

        PHIPRIM=PHIPRIM/(M4-M1+1)
        PHIPRIM2=PHIPRIM2/(M4-M1+1)
        PHIRMS=PHIPRIM2**.5
        PHIPRIM4=PHIPRIM4/(M4-M1+1)
        PHIPRIM6=PHIPRIM6/(M4-M1+1)
        RETURN
        END
...
****************************************************************************
        ...
        SUBROUTINE DISSIPATION(M1,M4,BB,Depsilon,ro2,DX,DM,igrid)
	integer*4 igrid
        real*8 BB(0:igrid)
...
10      CONTINUE
        ...
        Depsilon=DM*Depsilon/TN
        DD2=DD2/TN
        Dro1=Dro/TN
        Dro2=Dro1/(DD2*Depsilon)-1.
        RETURN
        END
...
****************************************************************************
        ...
        SUBROUTINE AVERAGE_1(sVA1ave,VA1int,sVA2ave,VA2int
     1  ,sVA3ave, VA3int, sVA4ave, VA4int, sVA5ave, VA5int,ipe,MR,mip)

        DIMENSION sVA1ave(mip), VA1int(mip)
        DIMENSION sVA2ave(mip), VA2int(mip)
        DIMENSION sVA3ave(mip), VA3int(mip)
        DIMENSION sVA4ave(mip), VA4int(mip)
        DIMENSION sVA5ave(mip), VA5int(mip)



        sVA1ave(ipe)=sVA1ave(ipe)*(MR-1)/MR+VA1int(ipe)/MR
        sVA2ave(ipe)=sVA2ave(ipe)*(MR-1)/MR+VA2int(ipe)/(MR)
        sVA3ave(ipe)=sVA3ave(ipe)*(MR-1)/MR+VA3int(ipe)/(MR)
        sVA4ave(ipe)=sVA4ave(ipe)*(MR-1)/MR+VA4int(ipe)/(MR)
        sVA5ave(ipe)=sVA5ave(ipe)*(MR-1)/MR+VA5int(ipe)/(MR)
